...
- **Group Activity**: one button on the side panel, one modal, one screen. The modal is all buttons, no typing: how many groups (2–6), groups of (pairs, 3s, 4s, 5s), or split by boys/girls or rows, each captioned with what it makes of *this* class today ("7–8 each", "15 groups") and greyed out with the reason when it can't. Tapping one deals the seated class into cards that slide over the board, with the side panel's pickers and +/- standing down the way they do for Swap Seats — the timer stays live. Building groups by hand is deliberately not a mode (thirty students is sixty taps); fix-ups are tap a chip then tap a card, or drag. Chips are name and homeroom number only, no avatars, sized from the card so two teams get big names and fifteen pairs small ones. Leftovers spread into existing groups rather than forming a group of one. Each card carries its own score; **Done** hands the points out the way the toggle at the bottom of the modal says, remembered per class: a star per point to every member, or one class point per group point straight onto the goal meter (only offered while a goal is live). The groups survive Done with points zeroed, so "Continue with Last Groups" picks the same teams up tomorrow; anyone no longer seated drops out on the way. Colours are fixed across themes (they *are* the group's identity); everything else follows the theme, like Flip Cards. Names default to Group 1, 2, 3 and rename is the one optional place to type.
